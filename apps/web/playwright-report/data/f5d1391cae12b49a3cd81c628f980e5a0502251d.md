...
- Name: ticket-approval-flow.spec.ts >> Ticket Approval Flow >> Frontend: ticket detail page renders
- Location: e2e\ticket-approval-flow.spec.ts:302:6
...
Call log:
  - navigating to "http://localhost:3000/tickets/72dc81a0-6c84-4734-bc2f-ea0ef0877f90", waiting until "domcontentloaded"
...
```ts
  205 | 		});
  206 | 		expect(httpRes.status()).toBe(201);
  207 | 		const { data } = await httpRes.json();
  208 | 		const ticketId = data.ticket_id;
  209 | 
  210 | 		// Wait for agent to reach a terminal state
  211 | 		const terminal = await pollUntilTerminal(
  212 | 			request,
  213 | 			ticketId,
  214 | 			60_000,
  215 | 		);
  216 | 
  217 | 		if (terminal.status === "pending_approval") {
