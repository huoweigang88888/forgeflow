import Link from "next/link";

export default function HomePage() {
	return (
		<main className="flex min-h-screen flex-col items-center justify-center p-8">
			<div className="max-w-2xl text-center">
				<h1 className="mb-4 text-5xl font-bold tracking-tight text-brand-600">
					ForgeFlow AI
				</h1>
				<p className="mb-2 text-xl text-slate-600">
					AI After-Sales Workforce for E-commerce
				</p>
				<p className="mb-8 text-slate-400">让电商售后处理效率提升 10 倍</p>
				<div className="flex gap-4 justify-center">
					<Link
						href="/dashboard"
						className="rounded-lg bg-brand-600 px-6 py-3 text-white hover:bg-brand-700 transition-colors"
					>
						Go to Dashboard →
					</Link>
					<a
						href="http://localhost:8000/docs"
						className="rounded-lg border border-slate-300 px-6 py-3 text-slate-700 hover:bg-slate-50 transition-colors"
						target="_blank"
						rel="noopener noreferrer"
					>
						API Docs
					</a>
				</div>
			</div>
		</main>
	);
}
